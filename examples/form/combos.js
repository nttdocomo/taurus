...
define(function (require) {
  ...
  $.mockjax(function (settings) {
    return {
      response: function (origSettings) {
        this.responseText = _.map(_.range(10), function () {
          return {
            'abbr': chance.word({length: 2}).toUpperCase(),
            'name': chance.name(),
            'slogan': chance.sentence({words: 3})
          }
        })
      }
    }
  ...
  var RemoteCollection = Backbone.Collection.extend({
	...
	var valueCollection = new Backbone.Collection(_.map(_.range(chance.natural({min: 1, max: 3})), function () {
		return {
			'abbr': chance.word({length: 2}).toUpperCase(),
			'name': chance.name(),
			'slogan': chance.sentence({words: 3})
